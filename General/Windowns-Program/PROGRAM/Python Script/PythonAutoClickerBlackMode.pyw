...
def update_ui():
    if clicking:
        ...
        status_label.config(text="StatusOn✅", foreground="lime")
    else:
        start_button.config(state=tk.NORMAL)
        stop_button.config(state=tk.DISABLED)
        status_label.config(text="StatusOFF❌", foreground="red")
...
def open_settings():
    ...
    ttk.Label(settings, text="Set Custom Toggle Key", font=("Segoe UI", 10, "bold"), foreground="white", background="black").pack(pady=10)
    key_display = ttk.Label(settings, text=f"Current: {custom_key}", font=("Segoe UI", 10), foreground="white", background="black")
    key_display.pack()
    ttk.Button(settings, text="Change Keybind", command=set_new_key).pack(pady=10)
    ttk.Label(settings, text="(Press the key you want after clicking)", font=("Segoe UI", 8), foreground="gray", background="black").pack(pady=5)
    settings.mainloop()
...
tk.Label(root, text="Inverted Auto Clicker", font=("Segoe UI", 14, "bold"), bg="black", fg="white").pack(pady=10)

frame = ttk.Frame(root)
frame.pack(pady=5)

ttk.Label(frame, text="Manual Speed (milliseconds):", foreground="white", background="black").pack(side=tk.LEFT, padx=5)

cps_entry = tk.Entry(frame, width=10, bg="black", fg="white", insertbackground="white")
...
preset_label = tk.Label(root, text="", font=("Segoe UI", 12, "bold"), bg="black", fg="white")
preset_label.pack()
...
def show_custom_dropdown():
    ...
    dropdown.configure(bg="#222222")

    for label, (value, color) in preset_options.items():
        item = tk.Label(dropdown, text=label, fg=color, bg="#222222", font=("Segoe UI", 10), anchor="w")
...
status_label = ttk.Label(root, text="StatusOFF❌", foreground="red", font=("Segoe UI", 10, "bold"), background="black")
...
hotkey_label = ttk.Label(root, text="Hotkey: F8 to toggle", font=("Segoe UI", 9), foreground="white", background="black")
hotkey_label.pack(pady=5)

ttk.Label(root, text="made by Tim & dimreal03", font=("Segoe UI", 8), foreground="white", background="black").pack(side=tk.BOTTOM, pady=10)
